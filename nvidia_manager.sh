...
function fetch_latest_version() {
    ...
    local ONLINE_VER=$(curl -sL --max-time 5 "https://www.nvidia.com/en-us/drivers/unix/" | grep -A 5 "Latest Production Branch Version" | grep -Eo "[0-9]{3}\.[0-9]{2}\.[0-9]{2}" | head -1)
    ...
    if [ -f /tmp/nvidia_latest_ver ]; then
        local NEW_VER=$(cat /tmp/nvidia_latest_ver)
        rm /tmp/nvidia_latest_ver
...
function handle_custom_version() {
    local INPUT=$(whiptail --title "Custom Version" --inputbox "Enter the specific Nvidia driver version you need (e.g., 535.183.01):" 10 60 "$DRIVER_VERSION" 3>&1 1>&2 2>&3)
...
function run_with_log() {
    ...
    while kill -0 $PID 2>/dev/null; do
        local LINE=$(tail -n1 "$LOGFILE" | cut -c1-70)
        PCT=$(( (PCT + 5) % 100 ))
        echo "XXX"; echo "$PCT"; echo "$LINE"; echo "XXX"
        sleep 0.5
    done | whiptail --title "$TITLE" --gauge "Please wait..." 10 80 0
...
function is_passthrough_enabled() {
    grep -q "lxc.mount.entry: /dev/nvidia0" "/etc/pve/lxc/${1}.conf"
}

function check_lxc_os_supported() {
    local CTID=$1
    # Check for 'apt' to confirm Debian/Ubuntu base
...
function handle_passthrough_setup() {
    ...
    local CTID=$(whiptail --title "Enable Passthrough" --menu "Select Container:" 15 60 6 "${MENU_ITEMS[@]}" 3>&1 1>&2 2>&3)
    if [ -z "$CTID" ]; then return; fi

    local MAJOR=$(get_nvidia_major_number)
    if [ -z "$MAJOR" ]; then
        whiptail --msgbox "Error: /dev/nvidia0 missing on host." 10 60; return
    fi
...
function handle_lxc_install() {
    ...
    local CTID=$(whiptail --title "Install Driver" --menu "Select Container:" 15 60 6 "${MENU_ITEMS[@]}" 3>&1 1>&2 2>&3)
    if [ -z "$CTID" ]; then return; fi
...
function handle_uninstall() {
    local CHOICE=$(whiptail --title "Uninstall Menu" --menu "Select Target:" 12 60 2 \
    ...
    case $CHOICE in
        1)
            ...
            ;;
        2)
            local CTID=$(whiptail --inputbox "Enter Container ID:" 10 60 3>&1 1>&2 2>&3)
            if [ -n "$CTID" ]; then
...
while true; do
    CHOICE=$(whiptail --title "Proxmox Nvidia Manager | by Sanchit Dang" --menu "Driver Version: $DRIVER_VERSION" 20 70 8 \
    "1" "Status Dashboard" \
